{% macro sparkhogwarts__snapshot_hash_arguments(args) -%}
    md5({%- for arg in args -%}
...
{% macro sparkhogwarts__snapshot_string_as_time(timestamp) -%}
    {%- set result = "to_timestamp('" ~ timestamp ~ "')" -%}
    {{ return(result) }}
{%- endmacro %}


{% macro sparkhogwarts__snapshot_merge_sql(target, source, insert_cols) -%}
...
{% macro sparkhogwarts__post_snapshot(staging_relation) %}
    {% do adapter.drop_relation(staging_relation) %}
{% endmacro %}


{% macro sparkhogwarts__create_columns(relation, columns) %}
    {% if columns|length > 0 %}
    {% call statement() %}
