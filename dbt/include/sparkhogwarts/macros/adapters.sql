...
{% macro sparkhogwarts__file_format_clause() %}
...
{% macro sparkhogwarts__location_clause() %}
...
{% macro sparkhogwarts__options_clause() -%}
...
{% macro sparkhogwarts__comment_clause() %}
...
{% macro sparkhogwarts__partition_cols(label, required=false) %}
...
{% macro sparkhogwarts__clustered_cols(label, required=false) %}
...
{% macro sparkhogwarts__create_temporary_view(relation, compiled_code) -%}
    create temporary view {{ relation }} as
      {{ compiled_code }}
{%- endmacro -%}


{%- macro sparkhogwarts__create_table_as(temporary, relation, compiled_code, language='sql') -%}
  {%- if language == 'sql' -%}
    {%- if temporary -%}
...
{% macro sparkhogwarts__persist_constraints(relation, model) %}
  {%- set contract_config = config.get('contract') -%}
...
{% macro sparkhogwarts__alter_table_add_constraints(relation, constraints) %}
...
{% macro sparkhogwarts__alter_column_set_constraints(relation, column_dict) %}
...
{% macro sparkhogwarts__create_view_as(relation, sql) -%}
...
{% macro sparkhogwarts__create_schema(relation) -%}
  {%- call statement('create_schema') -%}
    create schema if not exists {{relation}}
  {% endcall %}
{% endmacro %}

{% macro sparkhogwarts__drop_schema(relation) -%}
...
{% macro sparkhogwarts__get_columns_in_relation_raw(relation) -%}
...
{% macro sparkhogwarts__get_columns_in_relation(relation) -%}
...
{% macro sparkhogwarts__list_relations_without_caching(relation) %}
...
{% macro sparkhogwarts__list_schemas(database) -%}
...
{% macro sparkhogwarts__rename_relation(from_relation, to_relation) -%}
...
{% macro sparkhogwarts__drop_relation(relation) -%}
...
{% macro sparkhogwarts__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro sparkhogwarts__persist_docs(relation, model, for_relation, for_columns) -%}
  {% if for_columns and config.persist_column_docs() and model.columns %}
    {% do alter_column_comment(relation, model.columns) %}
  {% endif %}
{% endmacro %}

{% macro sparkhogwarts__alter_column_comment(relation, column_dict) %}
...
{% macro sparkhogwarts__make_temp_relation(base_relation, suffix) %}
...
{% macro sparkhogwarts__alter_column_type(relation, column_name, new_column_type) -%}
...
{% macro sparkhogwarts__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
