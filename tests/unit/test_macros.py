...
class TestSparkMacros(unittest.TestCase):
    def setUp(self):
        self.jinja_env = Environment(
            loader=FileSystemLoader("dbt/include/sparkhogwarts/macros"),
            extensions=[
                "jinja2.ext.do",
            ],
        )
    ...
    def __run_macro(self, template, name, temporary, relation, sql):
        ...
        def dispatch(macro_name, macro_namespace=None, packages=None):
            return getattr(template.module, f"sparkhogwarts__{macro_name}")
    ...
    def test_macros_create_table_as(self):
        ...
        sql = self.__run_macro(
            template, "sparkhogwarts__create_table_as", False, "my_table", "select 1"
        ).strip()
    ...
    def test_macros_create_table_as_hudi_options(self):
        ...
        sql = self.__run_macro(
            template, "sparkhogwarts__create_table_as", False, "my_table", "select 1 as id"
        ).strip()
        self.assertEqual(
